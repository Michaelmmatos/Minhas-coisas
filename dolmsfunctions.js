...
{
	console.log ("SCORM: doLMSInitialize()");

    var result;

	if(initialized) return;  // sewall 2015-01-09: now a no-op if called twice
    ...
    // alert("API.LMSInitialize(\"\") => " + result);  // sewall 2013-12-11
...
/**
 * Actually transmit saved results (score, suspend_data) to the LMS host.
 */
function doLMSCommit()
{
	console.log ("SCORM: doLMSCommit() typeof(API)", typeof(API), "typeof(parent.API)", typeof(parent.API), "typeof(parent.API.LMSCommit)", (parent.API ? typeof(parent.API.LMSCommit) : null));
...
function doLMSFinish()
{
	...
	{
		console.log ("Why are we here? initialized: " + initialized + ", finishCalled: " + finishCalled);
		return;
	}
    
    result = API.LMSFinish("");
    // alert("API.LMSFinish(\"\") => " + result);  // sewall 2013-12-11
...
*/
//window.onload = doLMSInitialize;
//window.onunload = doLMSFinish;
//window.onbeforeunload = doLMSFinish;


function doLMSGetValue(element)
{
	console.log ("SCORM: doLMSGetValue("+element+")");
    ...
    console.log ("API.LMSGetValue(" + element + ") => (" + result + "), API.LMSGetLastError(): " + API.LMSGetLastError());  // sewall 2013-11-01    
...
{
	console.log ("SCORM: doLMSSetValue("+element+","+value+")");
    ...
    console.log ("API.LMSSetValue(" + element + ", " + value + ") => " + result);   // sewall 2013-11-01    
    ...
    if (result == SCORM_FALSE)
	{
		...
		console.log ("Error - Could not store a value ("+element+") in the LMS.\n\nYour results may not be recorded.\n\n" + errorDescription);
		
        alert("Error - Could not store a value ("+element+") in the LMS.\n\nYour results may not be recorded.\n\n" + errorDescription);
...
/**
 * From http://stackoverflow.com/questions/105034/create-guid-uuid-in-javascript, contributed by Briguy37.
 */
